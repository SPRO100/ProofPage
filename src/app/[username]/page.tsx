...
import type { ProfileLink, ProjectStatus, RevenueMetric, RevenueSource } from "@/types/database";
...
export default async function PublicFounderPage({ params }: PageProps) {
  ...
  const { data: metrics } = projectIds.length ? await supabase.from("revenue_metrics").select("*").in("project_id", projectIds).order("recorded_at", { ascending: false }) : { data: [] };
  ...
  const displayProjects = projectList.map((project) => {
    ...
    return { ...project, revenue: buildRevenueDisplay(metric, source) };
  });
          ...
          <div className={styles.projectList}>
            ...
            {displayProjects.map((project, index) => {
              ...
              return <article className={styles.projectCard} key={project.id}>
                ...
                <div className={styles.metricRow}><div><span>Revenue</span><strong>{project.revenue ? formatMoney(project.revenue.mrr, project.revenue.currency) + "/mo" : "Not shared"}</strong></div><div><span>Proof</span><strong className={verified ? styles.green : styles.grey}>{verified ? "Verified" : "Unverified"}</strong></div><div><span>Source</span><strong>{verified && project.revenue?.provider ? project.revenue.provider : "Manual entry"}</strong></div></div>
                <div className={`${styles.chart} ${styles[index % 2 ? "steady" : "up"]}`} aria-hidden="true"><span /></div>
                <footer className={styles.cardFooter}><div className={verified ? styles.verifiedBadge : styles.unverifiedBadge}><span>{verified ? "✓" : "–"}</span>{verified ? "Revenue verified" : "Revenue not verified"}</div><time>{verified && project.revenue?.last_synced_at ? `Synced ${formatDate(project.revenue.last_synced_at)}` : "Not connected"}</time></footer>
              </article>;
            })}
          </div>
        </section>
